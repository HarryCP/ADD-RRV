...
addpath('~\BugTrap\Supply Tools\作图');
...
Node_init = [7, 3.5]';      %初始位置
% Node_goal = [0.5, 6.5]';    %目标位置
Node_goal = [9.5, 6.5]';    %目标位置

Robot_size = 0.1;   %机器人圆形半径
...
Num_obstacle_limit = 15;                %用于判断入口处的碰撞点是否足够丰富,10
Num_near_sample = 400;                  %在接触构型附近随机采样的样本点数量
...
for Iter_num = 1:Max_iter
    ...
    if Flag_new_entrance == 1
        ...
        Num_extend = 1;
        Num_extand_max = +inf;  %表示最大拓展步数+inf
        Node_type = 1;  %表示入口
...
% 输出结果
Solution = TreePlot(Tree, Total_node, Node_init, Node_goal, Success_num, Success, Obstacle);         %画出树
